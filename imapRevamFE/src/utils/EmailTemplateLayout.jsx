...
/** Match backend footer: show Salesforce Case id, not INC-#### */
function resolvePreviewSalesforceCaseId(data) {
  const fromHistory = data.history?.[0]?.incident_number;
  if (
    fromHistory != null &&
    String(fromHistory).trim() !== "" &&
    !DISPLAY_INCIDENT_LABEL.test(String(fromHistory).trim())
  ) {
    return String(fromHistory).trim();
  }
  const link = data.inputBox?.incidentLink;
  if (link) {
    const m = String(link).match(/\/Case\/([a-zA-Z0-9]+)\//);
    if (m) return m[1];
  }
  const input = data.inputBox?.inputNumber;
  if (
    input != null &&
    String(input).trim() !== "" &&
    !DISPLAY_INCIDENT_LABEL.test(String(input).trim())
  ) {
    return String(input).trim();
  }
  if (fromHistory != null && String(fromHistory).trim() !== "") {
    return String(fromHistory).trim();
  }
...
const EmailTemplateLayout = ({ data, previewDark = false }) => {
  ...
  const pc = previewDark
    ? {
        ...
        timelineTitle: "#cbd5e1",
        timelineMuted: "#334155",
        dotMuted: "#94a3b8",
        footerBg: "#080f1a",
        footerBottom: "#050a14",
    ...
    : {
        ...
        bodyText: "#1f2937",
        divider: "#F3F4F6",
        timelineBorder: "#E5E7EB",
        timelineTitle: "#374151",
        timelineMuted: "#E5E7EB",
        dotMuted: "#64748b",
        footerBg: "#F9FAFB",
        footerBottom: "#F3F4F6",
  ...
  const timelineNodePalette = (stepKey) =>
    TIMELINE_NODE[stepKey] || {
      fill: "#D1D5DB",
      line: "#E5E7EB",
      label: previewDark ? "#9CA3AF" : "#4b5563",
    };
  ...
  const salesforceCaseFooterId = resolvePreviewSalesforceCaseId(data);
  ...
  /* ── Shared styles ── */
  const labelStyle = {
    fontSize: "11px", fontWeight: "600", color: pc.label,
    letterSpacing: "0.5px", textTransform: "uppercase",
    marginBottom: "4px", fontFamily: "'Poppins', Arial, sans-serif",
  };
  const valueStyle = {
    fontSize: "14px", fontWeight: "500", color: pc.value,
  ...
  const emailPreview = (
    <div style={{
      width: "650px", margin: "0 auto", backgroundColor: pc.page,
      fontFamily: "'Poppins', Arial, sans-serif",
      border: `1px solid ${pc.pageBorder}`,
    }}>
      ...
      {data.radio.status !== "Not an Issue" &&
        ...
        <table width="100%" cellPadding="0" cellSpacing="0" style={{ borderCollapse: "collapse", borderBottom: `1px solid ${pc.timelineBorder}` }}>
          <tbody>
            <tr>
              <td
                style={{
              ...
              >
                {!flowTimelineImageSrc && (
                  <div style={{
                    fontSize: "11px", fontWeight: "700", color: pc.timelineTitle,
                  ...
                  <tbody>
                    <tr>
                      {remainingStatus.map((step, index) => {
                        ...
                        const circleColor = isPast ? pal.fill : pc.timelineMuted;
                        ...
                        const connectorColor = isCompleted ? segPal.line : pc.timelineMuted;
                        ...
                        return (
                          <td key={index} style={{
                            ...
                            <div style={{
                              ...
                              fontWeight: "700",
                              color: isPast ? "#FFFFFF" : pc.dotMuted,
                            ...
                            <div style={{
                              ...
                              color: isPast ? pal.label : pc.dotMuted,
                              fontFamily: "'Poppins', Arial, sans-serif",
                              lineHeight: "1.35",
                            }}>
                              {step.statusName}
      ...
      {data.statusUpdate === true && latestStatusUpdate?.length > 0 && (
        <table width="100%" cellPadding="0" cellSpacing="0" style={{ borderCollapse: "collapse" }}>
          <tbody><tr><td style={{ padding: "24px 36px 0 36px" }}>
            ...
            <div style={{
              backgroundColor: pc.greenBox.bg,
              borderLeft: pc.greenBox.border,
              padding: "14px 16px", fontSize: "14px", color: pc.greenBox.color, lineHeight: "1.6",
              fontFamily: "'Poppins', Arial, sans-serif",
            }}>
        ...
        <table width="100%" cellPadding="0" cellSpacing="0" style={{ borderCollapse: "collapse" }}>
          <tbody><tr><td style={{ padding: "24px 36px 0 36px" }}>
            ...
            {previousStatusUpdates.map((item, index) => (
              <div key={index} style={{
                backgroundColor: pc.amberBox.bg,
                borderLeft: pc.amberBox.border,
                padding: "12px 16px", fontSize: "14px", color: pc.amberBox.color, lineHeight: "1.6",
      ...
      <table width="100%" cellPadding="0" cellSpacing="0" style={{ borderCollapse: "collapse" }}>
        <tbody>
          ...
          <tr>
            ...
            <td style={{ width: "50%", padding: "0 36px 16px 16px", verticalAlign: "top", borderLeft: `1px solid ${pc.divider}` }}>
          ...
          {showResolvedOnRow ? (
            <tr>
              <td style={{ width: "50%", padding: "0 16px 16px 36px", borderTop: `1px solid ${pc.divider}`, paddingTop: "16px", verticalAlign: "top" }}>
                <div style={labelStyle}>Resolved On (UTC)</div>
                <div style={{ ...valueStyle, color: previewDark ? "#6ee7b7" : "#16A34A" }}>{formatDate(data.dateTime.resolvedTime?.utc) || "—"}</div>
              </td>
              <td style={{ width: "50%", padding: "0 36px 16px 16px", borderTop: `1px solid ${pc.divider}`, paddingTop: "16px", borderLeft: `1px solid ${pc.divider}`, verticalAlign: "top" }}>
            ...
            <tr>
              <td colSpan="2" style={{ padding: "0 36px 16px 36px", borderTop: `1px solid ${pc.divider}`, paddingTop: "16px" }}>
                <div style={labelStyle}>Incident Type</div>
                <div style={valueStyle}>{incidentTypeDisplay}</div>
              </td>
            </tr>
          ...
          <tr>
            <td style={{ width: "50%", padding: "0 16px 16px 36px", borderTop: `1px solid ${pc.divider}`, paddingTop: "16px", verticalAlign: "top" }}>
              ...
              <div style={{
                ...
                backgroundColor: severityUi[data.dropDown.severity]?.bgColor || (previewDark ? "rgba(255,255,255,0.06)" : "#F3F4F6"),
                color: severityUi[data.dropDown.severity]?.color || pc.value,
                border: `1px solid ${severityUi[data.dropDown.severity]?.borderColor || pc.pageBorder}`,
          ...
          {["suspected", "ongoing", "resolved", "resolved-rca"].includes(normalizedStatus) && (
            <tr>
              ...
              ) : <td style={{ width: "50%", borderTop: `1px solid ${pc.divider}` }} />}
            </tr>
          )}
        </tbody>
      </table>
      ...
      {data.radio?.workaround === "Yes" && data.textArea?.workaroundDetails && (
        <table width="100%" cellPadding="0" cellSpacing="0" style={{ borderCollapse: "collapse" }}>
          <tbody><tr><td style={{ padding: "24px 36px 0 36px" }}>
            ...
            <div style={{ backgroundColor: pc.greenBox.bg, borderLeft: pc.greenBox.border, padding: "14px 16px", fontSize: "14px", color: pc.greenBox.color, lineHeight: "1.6" }}>
      ...
      {normalizedStatus === "resolved-rca" && data.textArea.resolvedwithRcaDetails && (
        <table width="100%" cellPadding="0" cellSpacing="0" style={{ borderCollapse: "collapse" }}>
          <tbody><tr><td style={{ padding: "24px 36px 0 36px" }}>
            ...
            <div style={{ backgroundColor: pc.blueBox.bg, borderLeft: pc.blueBox.border, padding: "14px 16px", fontSize: "14px", color: pc.blueBox.color, lineHeight: "1.6" }}>
      ...
      <table width="100%" cellPadding="0" cellSpacing="0" style={{ borderCollapse: "collapse", marginTop: "28px" }}>
        <tbody>
                ...
                <tbody><tr>
                  ...
                  <td style={{ paddingLeft: "8px" }}>
                    <button
                      ...
                      style={{
                        display: "inline-block", padding: "9px 22px", backgroundColor: pc.btnSecondaryBg,
                        border: `1px solid ${pc.btnSecondaryBorder}`, color: pc.btnSecondaryText, borderRadius: "6px",
          ...
          {/* Team + ID */}
          <tr>
            <td style={{ backgroundColor: pc.footerBg, padding: "0 36px 18px 36px", verticalAlign: "middle" }}>
              <div style={{ fontSize: "13px", fontWeight: "600", color: pc.footerStrong, fontFamily: "'Poppins', Arial, sans-serif" }}>Taboola Incident Management</div>
              <div style={{ fontSize: "11px", color: pc.footerSmall, marginTop: "2px", fontFamily: "'Poppins', Arial, sans-serif" }}>IncidentManagement@taboola.com</div>
            </td>
            <td style={{ backgroundColor: pc.footerBg, padding: "0 36px 18px 36px", textAlign: "right", verticalAlign: "middle" }}>
              <div style={{ fontSize: "10px", color: pc.footerSmall, textTransform: "uppercase", letterSpacing: "1px", marginBottom: "2px", fontFamily: "'Poppins', Arial, sans-serif" }}>Salesforce Case ID</div>
              <div style={{ fontSize: "13px", fontWeight: "700", color: pc.footerStrong, fontFamily: "'Poppins', Arial, sans-serif" }}>{salesforceCaseFooterId}</div>
            </td>
          </tr>

          <tr>
            <td colSpan="2" style={{ backgroundColor: pc.footerBottom, padding: "12px 36px", textAlign: "center" }}>
              <div style={{ fontSize: "10px", color: pc.footerSmall, fontFamily: "'Poppins', Arial, sans-serif" }}>
